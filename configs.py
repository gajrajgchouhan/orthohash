...
def nclass(config):
    r = {"imagenet100": 100, "cifar10": 10, "nuswide": 21, "coco": 80}[config["dataset"]]

    return r


def R(config):
    r = {"imagenet100": 1000, "cifar10": 59000, "cifar10_2": 50000, "nuswide": 5000, "coco": 5000}[  # mAP@all
...
def optimizer(config, params):
    ...
    if o_type == "sgd":
        ...
        o = Adam(
            params, lr=kwargs["lr"], betas=kwargs.get("betas", (0.9, 0.999)), weight_decay=kwargs.get("weight_decay", 0)
        )
...
def scheduler(config, optimizer):
    ...
    if s_type == "step":
        return lr_scheduler.StepLR(optimizer, kwargs["step_size"], kwargs["gamma"])
    elif s_type == "mstep":
        return lr_scheduler.MultiStepLR(
            optimizer, [int(float(m) * int(config["epochs"])) for m in kwargs["milestones"].split(",")], kwargs["gamma"]
        )
    else:
        raise Exception("Scheduler not supported yet: " + s_type)


def compose_transform(mode="train", resize=0, crop=0, norm=0, augmentations=None):
    """
...
def dataset(config, filename, transform_mode):
    ...
    if dataset_name in ["imagenet100", "nuswide", "coco"]:
        ...
        datafunc = {"imagenet100": datasets.imagenet100, "nuswide": datasets.nuswide, "coco": datasets.coco}[
            dataset_name
        ]
...
def dataloader(d, bs=256, shuffle=True, workers=-1, drop_last=True) -> DataLoader:
    if workers < 0:
        workers = default_workers
    l = DataLoader(d, bs, shuffle, drop_last=drop_last, num_workers=workers)
    return l
