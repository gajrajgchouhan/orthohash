    ...
    def forward(self, logits, code_logits, labels, onehot=True):
        if self.multiclass:
            ...
            if self.multiclass_loss in ["bce", "imbalance"]:
                ...
                if self.multiclass_loss == "imbalance":
                    imbalance_mask, multiclass_acc = get_imbalance_mask(
        ...
        if self.quan != 0:
            if self.quan_type == "cs":
                quantization = 1.0 - F.cosine_similarity(code_logits, code_logits.detach().sign(), dim=1)
            ...
            quantization = torch.tensor(0.0).to(code_logits.device)
